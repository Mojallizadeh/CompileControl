 ...
 *      minor modifications.
         aviFail[0] = calcOutputController(rUserVar1, rUserVar2, rUserVar3, rUserVar4, rUserVar5, rUserVar6, rUserVar7, rUserVar8, rUserVar9, rUserVar10, rInit, rGeneratorSpeed, rRotorSpeed, rRatedSpeed,
                        rBelowRatedPitch, rForeAftTower, rSideTower,
                        rRotorAzimuth, rOP1RootMoment, rOP2RootMoment,
                        rOP3RootMoment, rMeasuredPitch, rMeasuredPPR, rWindSpeed, rMeasuredTorque, 
						rWaves_prev_1, rWaves_prev_2, rWaves_prev_3, rWaves_prev_4, rWaves_prev_5, rWaves_prev_6,
                        rModeGain, &rTorqueDemand, &rBlade1Pitch, 
                        &rBlade2Pitch, &rBlade3Pitch, errorMsg);
 */
#if !defined(MULTITASKING)  /* SINGLETASKING */
DISCON_LOCAL int calcOutputController(float rInit, float rGeneratorSpeed, float rRotorSpeed, float rRatedSpeed,
        float rBelowRatedPitch, float rForeAftTower, float rSideTower,
        float rRotorAzimuth, float rOP1RootMoment, float rOP2RootMoment,
        float rOP3RootMoment, float rMeasuredPitch, float rMeasuredPPR, float rWindSpeed, float rMeasuredTorque, 
    ...
    SIG_MODEL(U,Rotor_Speed) = rRotorSpeed;
    ...
    SIG_MODEL(U,Measured_PPR) = rMeasuredPPR;
    SIG_MODEL(U,Measured_WS) = rWindSpeed;
...
float* SetParams(float *avrSwap, char *accInfile) 
{
	char mystring [200];
	int iStatus;
	...
	if (iStatus == 0)
	{
		pFile = fopen (accInfile,"r");
		if (pFile == NULL) {
            avrSwap[128]=1;
            printf("ERROR! Input controller file not found\n\n\n\n");
        }
		else {
...
void DISCON_API CDECL DISCON(float *avrSwap, int *aviFail,
    ...
    float rTime, rSample, rGeneratorSpeed, rRotorSpeed, rRatedSpeed, rBelowRatedPitch, 
            rRotorAzimuth, rOP1RootMoment, rOP2RootMoment, rOP3RootMoment,
            rForeAftTower, rSideTower, rMeasuredPitch, rMeasuredPPR, rWindSpeed, rMeasuredTorque,
rInit,  rModeGain;
	...
	SetParams(avrSwap, accInfile); /*PF disable this call for Labview's sake*/
    ...
    //printf("write to swap array\n");
	//printf("     DISCON force X = %f\n", rWaves_prev_5);
    ...
    if (iStatus == 0) {
        ...
        /* Main calculation */
        aviFail[0] = calcOutputController(rInit, rGeneratorSpeed, rRotorSpeed, rRatedSpeed,
                        rBelowRatedPitch, rForeAftTower, rSideTower,
                        rRotorAzimuth, rOP1RootMoment, rOP2RootMoment,
                        rOP3RootMoment, rMeasuredPitch, rMeasuredPPR, rWindSpeed, rMeasuredTorque, 
                        rModeGain, &rTorqueDemand, &rBlade1Pitch, 
